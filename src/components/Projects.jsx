  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <h4 className="font-display text-base font-semibold text-neutral-900 dark:text-white">{p.name}</h4>
          <div className="font-mono text-[11px] text-neutral-500 dark:text-neutral-500 mt-0.5">{p.tag}</div>
        </div>
        <ExternalLink className="w-4 h-4 text-neutral-400 group-hover:text-[#F2B84B] transition-colors shrink-0 mt-1" />
      </div>
      <p className="text-[15px] leading-6 text-neutral-700 dark:text-neutral-300 mt-3">{p.desc}</p>
      ...
      <div className="font-mono text-[11px] text-[#B4790F] dark:text-[#F2B84B] mt-4 group-hover:underline">↳ {p.link}</div>
    </a>
  );
}
...
export default function Projects() {
  return (
    <section id="projects" className="py-16 px-6 lg:px-10 max-w-5xl mx-auto scroll-mt-14">
      <Reveal>
